...
#include "Misc.h"
Sprite3D::Sprite3D(ID3D11Device* device, const wchar_t* filename)
{
    ...
    hr = textureManager->loadTextureFromFile(device, filename, shaderResourceView.ReleaseAndGetAddressOf(), &texture2dDesc);
    _ASSERT_EXPR(SUCCEEDED(hr), hr_trace(hr));
